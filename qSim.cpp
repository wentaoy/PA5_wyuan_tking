...
int main(int argc, char *argv[]) {
	 ...
	 *************************************************************************************************/
	 ...
	 EventQueuePrior* eqCommon = new EventQueuePrior();
	 EventQueuePrior* eqMuti = new EventQueuePrior();
	 ...
	 //Create customer arrival event and insert into eventqueuePrior
	 ...
	 //create Tellerevent and insert into both eventQueuePrior
	 ...
	 float worldTime = 0;
	 while ((int)completedCusCommon->size() < customerNum) {
	 eqCommon->getHead()->action();
	 worldTime = eqCommon->getHead()->getTime();
	 eqCommon->deleteHead();
	 }
	 //destructing eqCommon
	 eqCommon->~EventQueuePrior();
	 ...
	 for(int i = 0; i < (int)completedCusCommon->size(); i++){
	 ...
	 // start mutiline simulation


	 worldTime = 0;
	 while ((int)completedCusMuti->size() < customerNum){//!eqMuti->isEmpty() && worldTime < simulateTime && (int)completedCusMuti->size() < customerNum) {
	 eqMuti->getHead()->action();
	 worldTime = eqMuti->getHead()->getTime();
	 eqMuti->deleteHead();
	 }

	 eqMuti->~EventQueuePrior();
	 ...
	 for(int i = 0; i < (int)completedCusMuti->size(); i++){
	 //if this wait time is greater than all before set it as max
